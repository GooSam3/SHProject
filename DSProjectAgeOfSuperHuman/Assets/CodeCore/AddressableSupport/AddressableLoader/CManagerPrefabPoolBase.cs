...
{   
	...
	/// </summary>
    protected void ReserveInstance(string _category, string _addressableName, int _reserveCount, UnityAction<string, GameObject> _eventFinish, bool _autoRelease)
	{
    ...
    protected void ReturnClone(GameObject _returnObject)
	{
		...
		if (Prefab != null)
		{
			...
			if (Prefab.AutoRelease)
			{
				...
				{
					RemoveInstance(Prefab.Category, Prefab.AddressableName, false);
				}
			}
		}		
	}

    protected void RemoveInstance(string _category, string _addressable, bool bOnlyRemovePrefab)
	{
		Dictionary<string, SPrefabPool> PrefabInstance = FindPrefabPoolOrAlloc(_category);
	...
	protected void RemoveInstanceAllSoft()  // 프리팹만 해재한다. 에셋번들은 유지한다.
	{
		Dictionary<string, Dictionary<string, SPrefabPool>>.Enumerator it = m_dicPrefabInstance.GetEnumerator();
    ...
    private void RemoveInternal(SPrefabPool _removePrefab, bool bOnlyRemovePrefab)
	{
