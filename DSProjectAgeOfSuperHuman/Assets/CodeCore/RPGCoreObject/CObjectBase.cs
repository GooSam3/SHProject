...
abstract public class CObjectInstanceBase : object
{
    public TEMPLATE CopyInstance<TEMPLATE>() where TEMPLATE : CObjectInstanceBase
	{
        TEMPLATE pNewInstance = MemberwiseClone() as TEMPLATE;

        Type pType = pNewInstance.GetType();

        FieldInfo[] aFieldInfo = pType.GetFields();
        for (int i = 0; i < aFieldInfo.Length; i++)
		{
            FieldInfo pFieldInfo = aFieldInfo[i];
            CObjectInstanceBase pMemeber = pFieldInfo.GetValue(this) as CObjectInstanceBase;
            if (pMemeber != null)
			{
        ...
        OnObjectCopy(pNewInstance);
        return pNewInstance;
    }

    public CObjectInstanceBase CopyInstance()
	{
        CObjectInstanceBase pNewInstance = MemberwiseClone() as CObjectInstanceBase;
        OnObjectCopy(pNewInstance);
        return pNewInstance;
    }

    protected virtual void OnObjectCopy<TEMPLATE>(TEMPLATE pIntance) where TEMPLATE : CObjectInstanceBase { }
}
