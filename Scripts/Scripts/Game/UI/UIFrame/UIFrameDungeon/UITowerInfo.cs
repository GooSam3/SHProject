...
/// 시공의 틈
/// </summary>
public class UITowerInfo : MonoBehaviour, ITabContents
{
    [SerializeField] private GameObject rewardGroup;
    [SerializeField] private GameObject rewardRuneGroup;
	...
	{
		timeInfoDesc.text = DBLocale.GetText("Despair_Time_Desc");
        ...
        rewardRuneGroup.SetActive(false);
	...
	{
		var towerList = DBStage.GetStageList(E_StageType.Tower);
		towerList.Sort((a, b) => {
	...
	private void DoInfoUpdate(bool isResetPos)
	{
		...
		// 남은시간 표시
		ulong remainderTime = 0;
		var abilityAction = ZPawnManager.Instance.MyEntity.FindAbilityAction(stageTable.InBuffID);
		if (abilityAction != null) {
			remainderTime = abilityAction.EndServerTime - TimeManager.NowSec;
		}
		else {
			ZLog.LogError(ZLogChannel.Default, $"EntityAbilityAction 이 null이다, InBuffID:{stageTable.InBuffID }");
		}

		if (remainderTime > 0) {
			UpdateRemainderTime(remainderTime);
		}
		else {
			timeInfoText.text = DBLocale.GetText("Nothing_Label");
		}

		// 스테이지 잠김 표시 여부
		if (ZNet.Data.Me.CurCharData.LastLevel >= stageTable.InMinLevel) {
			dungeonLockImage.SetActive(false);

			startButton.interactable = remainderTime > 0 && ZGameModeManager.Instance.CurrentGameModeType != E_GameModeType.Tower;
		}
		else {
			dungeonLockImage.SetActive(true);
			dungeonLockMsgText.text = string.Format(DBLocale.GetText("BossDungeon_JoinLv"), stageTable.InMinLevel);
	...
	private void DoRewardRuneItemUpdate(Stage_Table stageTable, bool isResetPos)
	{
        ...
        rewardRuneGroup.SetActive(dropItemList.Count > 0);
    }
	...
	private void EnterDungeon()
	{
		...
		ulong ivenItemId = 0;
		var invenItem = ZNet.Data.Me.CurCharData.GetInvenItemUsingMaterial(portalTable.UseItemID);
		if (invenItem != null) {
			ivenItemId = invenItem.item_id;
		}

		AudioManager.Instance.PlaySFX(30004);
		ZGameManager.Instance.TryEnterStage(portalTable.PortalID, true, ivenItemId, portalTable.UseItemID);
