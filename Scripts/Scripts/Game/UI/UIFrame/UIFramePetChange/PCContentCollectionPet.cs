...
public class PCContentCollectionPet : PCContentCollectionBase
{
    protected override int CurCollection => Me.CurCharData.GetCompleteCollectItems(CollectionType.TYPE_PET)?.Count ?? 0;

    protected override int MaxPCR => DBPet.GetAllPetData().Count;

    protected override int CurPCR => Me.CurCharData.GetPetDataList().Count;

    protected override List<UIAbilityData> GetCollectionAbilityList()
    {
        List<UIAbilityData> listAbility = new List<UIAbilityData>();

        var list = Me.CurCharData.GetCompleteCollectItems(CollectionType.TYPE_PET);
        ...
        {
            if (DBPetCollect.GetPetCollection(iter.CollectTid, out var table) == false)
                continue;
    ...
    protected override List<ScrollPCRCollectionData> GetSortedData(E_CollectionSortType type)
    {
        ...
        switch (type)
        {
            case E_CollectionSortType.Completed:
                ...
                {
                    var collectData = Me.CurCharData.GetCollectData(iter.Tid, CollectionType.TYPE_PET);
    ...
    protected override List<ScrollPCRCollectionData> GetSortedData(string str)
    {
        ...
        foreach (var iter in BaseContentData)
        {
            if (DBPetCollect.GetPetCollection(iter.Tid, out var collectTable) == false)
                continue;
    ...
    protected override void InitilaizeList()
    {
        ...
        foreach (var data in DBPetCollect.GetPetCollectionDatas())
        {
            BaseContentData.Add(new ScrollPCRCollectionData(E_PetChangeViewType.Pet, data.PetCollectionID, data.Sort));
        }
