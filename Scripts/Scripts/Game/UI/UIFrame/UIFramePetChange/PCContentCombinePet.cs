...
public class PCContentCombinePet : PCContentCombineBase
{
    // 소유중인, 고대가 아닌|
    ...
    public override void SetSortedContent(E_PetChangeSortType sortType)
    {
        ...
        if (isViewNotice)
        {
            ...
            {
                TextNotice.text = DBLocale.GetText("WPet_PetCompose_6T");
            }
            else
            {
    ...
    protected override long GetCombineCost(int multi) => DBCompose.GetPetComposeCost((byte)RegistedGrade) * multi;

    protected override int GetCombineCount() => DBCompose.GetPetComposeCount((byte)RegistedGrade);


    public override void OnClickCombine()
    {
        if (DBCompose.GetPetComposeTable((byte)RegistedGrade, out var table) == false)
            return;
        ...
        ZWebManager.Instance.WebGame.REQ_ComposePet(table.PetComposeID, (uint)count, CombineList.listCombineData, (recvPacket, recvMsgPacket) =>
        {
            ...
            else
            {
                ...
                {
                    UIManager.Instance.Find<UIFramePet>()?.SetReserveContent(UIFramePetChangeBase.E_PetChangeContentType.Content_2);
                    frame.SetCombineResult(UIGachaEnum.E_GachaStyle.Pet, UIGachaEnum.E_TimeLineType.Pet_10_Start, listcompose);
                });
                return;
            }
        });
